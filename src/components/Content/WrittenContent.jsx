...
const WrittenContent = ({ category, indexValue }) => {
  ...
  // I was not sure about this so i haven't removed this
  // const handleHtml = (str) => {
  //   const parser = new DOMParser();
  //   const hmtl = parser.parseFromString(str, "text/html");
  //   var arr = Array.from( hmtl)
  //   return "dd";
  // };
      ...
      {category === "Blogs" && (
        <div>
          ...
          <p className="font-satoshi-bold text-desc-custom leading-20">
            {parse(blogs[indexValue].description)}
            {/* {handleHtml(blogs[indexValue].description)} */}
          </p>
        </div>
      )}
      {category === "Articles" && (
